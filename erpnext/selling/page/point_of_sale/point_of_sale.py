...
@frappe.whitelist()
def get_items(start, page_length, price_list, item_group, pos_profile, selected_customer_name = "", search_term=""):
	...
	#Adding  custom filters  from  frontend
	custom_filters = """
			item.disabled = 0
	...
	if selected_customer_name != '':
		custom_filters = """
			...
			AND item.is_sales_item = 1
			AND item.is_fixed_asset = 0
			AND item.linked_customer = '{selected_customer_name}'
			AND item.item_group in (SELECT name FROM `tabItem Group` WHERE lft >= {lft} AND rgt <= {rgt})
			AND {condition}
			{bin_join_condition}
			""".format(
...
def get_product_bundle(item_code):
	'''
	Function that checks if a product bundle exist for a 
	given item and returns it
	'''
	if bool(frappe.db.exists("Product Bundle", {"new_item_code": item_code})):
...
def get_milling_charge_per_uom():
	'''
	Getting the milling charge value form the feeds settings
	'''
	pass

@frappe.whitelist()
def get_product_bundle_n_prices(item_code):
	# get product bundle
	product_bundle = get_product_bundle(item_code)
	if not product_bundle.get('status'):
		return {'status': False}

	# get milling charge
	milling_charge_per_uom = frappe.get_doc("Feeds Settings").milling_charge_per_uom
	...
	for package_item in product_bundle.get('doc').atomic_items:
		...
		if not package_item_price.get('status'):
			bundle_items_n_prices[package_item.item_code] = {
	...
	return {
		...
		'milling_charge_per_uom': milling_charge_per_uom
	}
